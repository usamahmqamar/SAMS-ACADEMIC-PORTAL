...
// 2. Multi-branch verification check
...
app.post('/api/reset_session_data', (req, res) => {
  try {
    dbState = JSON.parse(JSON.stringify(INITIAL_DB_STATE));
    saveDB(dbState);
    res.json({
      success: true,
      message: "Database successfully reset to clean 2026/2027 Academic Session slate.",
      session: "2026/2027 Academic Session",
      timestamp: new Date().toISOString()
    });
  } catch (err: any) {
    console.error("Session reset failed:", err);
    res.status(500).json({ error: "Failed to reset session data", details: err?.message });
  }
});
